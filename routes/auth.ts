import { bcrypt, REDIRECT_BACK, Router } from '../deps.ts';

import type { CustomState } from '../lib/types.ts';
import { userValidateLogin, userValidateSignup } from '../lib/utils.ts';
...
router
  ...
  .post('/signup', async ({ request, response, app, state }) => {
    const appState = <CustomState> app.state;
  ...
  .post('/login', async ({ request, response, app, state }) => {
    const appState = <CustomState> app.state;
    const params: URLSearchParams = await request.body().value;
    const user = {
      username: params.get('username'),
